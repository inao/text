...
基本は名詞的表現とし、どういった箇所でのみ動詞的表現を用いるかを決めるのがスムーズです。
...
`hoge`のようにバッククォート記号で括り`<code>`タグ扱いにする基準は全体で統一されていれば大丈夫です。

一般的には、次のようなものをバッククォート記号で括ります。
...
「一つ」なのか「1つ」なのかは、「2つ」や「3つ」に言い換えられるかどうかで判断できます。
「プログラミング言語の一つ」は、「プログラミング言語の三つ」とは言わないので慣用表現です。漢数字にしてください。
「1つの変数」は、「3つの変数」とは言うので算数字です。
